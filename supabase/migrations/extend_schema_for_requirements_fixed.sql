...
-- 既存のポリシーを削除してから再作成
DROP POLICY IF EXISTS "全従業員が年度マスタを閲覧可能" ON fiscal_years;
CREATE POLICY "全従業員が年度マスタを閲覧可能" ON fiscal_years
  FOR SELECT USING (true);

DROP POLICY IF EXISTS "管理者のみが年度マスタを編集可能" ON fiscal_years;
...
-- 既存のポリシーを削除してから再作成
DROP POLICY IF EXISTS "全従業員が拠点マスタを閲覧可能" ON branches;
CREATE POLICY "全従業員が拠点マスタを閲覧可能" ON branches
  FOR SELECT USING (true);

DROP POLICY IF EXISTS "管理者のみが拠点マスタを編集可能" ON branches;
